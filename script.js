const MOT_DE_PASSE_ADMIN = "Mustchrist6@";
let logoOui = false;
...
function traduireType(v) {
  return {
    ...
    "vitrine": "Site vitrine entreprise",
...
function traduireStyle(v) {
  return {
    ...
    "colore": "Coloré et dynamique",
    "sombre": "Sombre et professionnel"
  }[v] || v;
}

function traduireLogo(v) {
  return {
    "oui": "Oui, j'ai un logo",
    "non": "Non, je n'ai pas de logo",
    "creation": "Non, mais je souhaite en créer un"
  }[v] || v;
}

function traduireBudget(v) {
  return {
    "5000-15000": "5 000 — 15 000 FCFA",
    "15000-50000": "15 000 — 50 000 FCFA",
    "50000-100000": "50 000 — 100 000 FCFA",
    "100000+": "Plus de 100 000 FCFA",
    "discuter": "A discuter"
  }[v] || v;
}
...
function traduireDelai(v) {
  return {
    ...
    "normal": "Normal — 1 à 2 semaines",
    "flexible": "Flexible — plus de 2 semaines"
  }[v] || v;
}
...
function soumettre() {
  ...
  document.getElementById("numeroCommande").innerHTML = "CMD-" + d.numero.split("-")[1];
...
function afficherPageAdmin() {
  ...
  document.getElementById("stats-admin").innerHTML = `
    <div class="stats-grid">
      ...
      <div class="stat-carte"><div class="stat-nombre">${visites}</div><div class="stat-label">Visites totales</div></div>
    </div>`;
  ...
  if (commandes.length === 0) {
    ...
    commandes.slice().reverse().forEach(function(c, i) {
      let index = commandes.length - 1 - i;
      html += `
        <div class="commande-carte">
          ...
          <div class="commande-actions">
            <button class="btn-telecharger" onclick="telechargerCommande(${index})">Télécharger</button>
            <button class="btn-supprimer" onclick="supprimerCommande(${index})">Supprimer</button>
          </div>
          <div style="margin-top:12px; border-top:1px solid #333; padding-top:12px;">
            ...
            <input type="text" class="champ-lien" id="lien-${index}"
            value="${c.lienSite||''}"
            placeholder="Ex: https://nomduclient.netlify.app" />
            <button class="btn-enregistrer" onclick="enregistrerLien(${index})">
            Enregistrer le lien
            </button>
          </div>
        </div>`;
...
function enregistrerLien(index) {
  let lien = document.getElementById("lien-" + index).value;
  if (!lien) {
    alert("Veuillez entrer le lien du site !");
    return;
  }
  let commandes = JSON.parse(localStorage.getItem("commandes") || "[]");
  commandes[index].lienSite = lien;
  localStorage.setItem("commandes", JSON.stringify(commandes));
...
function rechercherCommande() {
  ...
  let commandes = JSON.parse(localStorage.getItem("commandes") || "[]");
  let commande = commandes.find(function(c) {
...
function telechargerCommande(index) {
  let commandes = JSON.parse(localStorage.getItem("commandes") || "[]");
  let c = commandes[index];
...
Details specifiques : ${JSON.stringify(sp, null, 2)}
...
let visites = parseInt(localStorage.getItem("visites") || "0");
localStorage.setItem("visites", visites + 1);
